...
public class InviteCommand extends ConsoleCommand {
    ...
    @Override public void execute(String command, PrintStream out) {
        try {
            List<JID> jids = Arrays.asList(command.split(" ")).stream().skip(1).map(x -> new JID(x)).collect(Collectors.toList());
